...
> Folder structure options and naming conventions for Web-UI projects

### A typical top-level directory layout

    .
    ├── build                   # Compiled files (alternatively `dist`). Gets generated
    ├── docs                    # Documentation files (alternatively `doc`)
    ├── public                  # HTML files (things the public would see)
    ├── resources               # Code to use as a resource. Does not get compiled into the build
...
Rich Text Format (RTF) documentation, which is usually stored into the `docs` folder.
